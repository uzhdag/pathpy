...
class MultiOrderModel:
    ...
    def likelihood(self, paths, max_order=None, log=True):
        ...
        """
        max_order = self.maxOrder if max_order is None else max_order
        assert max_order <= self.maxOrder, \
    ...
    def degrees_od_freedom(self, max_order=None, assumption="paths"):
        """
    ...
    def likelihood_ratio_test(self, paths, max_order_null=0, max_order=1,
        ...
        x = -2 * (self.likelihood(paths, max_order=max_order_null, log=True) - self.likelihood(paths, max_order=max_order, log=True))

        # we calculate the additional degrees of freedom in the alternative model
        dof_diff = self.degrees_od_freedom(max_order=max_order, assumption=assumption) - self.degrees_od_freedom(
            max_order=max_order_null, assumption=assumption)
    ...
    def test_network_hypothesis(self, paths, method='AIC'):
        ...
        L0, n0 = self.layer_likelihood(paths, l=0, consider_longer_paths=True, log=True, min_path_length=1)

        # log-likelihood and observation count of first-order model
        L1, n1 = self.layer_likelihood(paths, l=1, consider_longer_paths=True, log=True, min_path_length=1)
