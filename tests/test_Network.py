...
def test_remove_node(random_network, directed, weighted):
    ...
    assert net.vcount() == vc-1
    assert net.ecount() == ec - len(incident_edges)
    assert net.total_edge_weight() == tw - weight_incident
...
def test_add_edge(random_network, directed, weighted):
    ...
    assert net.vcount() == vc
    assert net.ecount() == ec + 1
    assert net.total_edge_weight() == tw + weight_to_add
